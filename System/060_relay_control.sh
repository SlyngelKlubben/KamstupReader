...
off_light_level double precision,
pin_state text,
pin_expire time with time zone
)
RETURNS text AS
$$
BEGIN
...
SELECT public.relay_state(
 task::text, now()::time with time zone, off_time_start::time with time zone, off_time_end::time with time zone, envi.light::double precision, off_light_level::double precision, pin_state::text, pin_expire::time with time zone) 
